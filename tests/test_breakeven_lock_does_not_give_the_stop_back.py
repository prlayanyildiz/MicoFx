"""BE-1: a paper lock at entry, independent of the trail.

Live has no separate breakeven step — the trail crosses entry only after
gain exceeds trail_step_atr (test_trail_breakeven_invariant). Autopsy rows
then show a GER40 trade at +1.42 R closing −1.00 R because trail_step (2.2)
is wider than the stop (1.0).

``breakeven_at_r=0`` is today. Search and walk_forward must not pass it.
"""
...
def test_simulate_takes_breakeven_at_r_default_off():
    sig = inspect.signature(backtest.simulate)
    assert "breakeven_at_r" in sig.parameters
    assert sig.parameters["breakeven_at_r"].default == 0.0
