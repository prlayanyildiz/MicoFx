"""cost_rank_max upgrades from charged holdout — entry gate only."""
...
def best_cost_rank_upgrade(
    ...
    *,
    ...
    for cap, hold in scored.items():
        if abs(float(cap) - float(live_cr)) < 1e-9 and isinstance(hold, dict):
            live_hold = hold
            break
    ...
    for cap, hold in scored.items():
        if not isinstance(hold, dict):
            continue
        if abs(float(cap) - float(live_cr)) < 1e-9:
            continue
        try:
        ...
        if not _neighbor_supported(
            float(cap), scored, net_r, max_gap_r=max_neighbor_gap_r,
        ):
            continue
        key = (-net_r, abs(float(cap) - float(live_cr)))
        if best is None or key < best_key:
            best_key = key
            best = {
                "cost_rank_max": float(cap),
                "net_r": net_r,
                "profit_factor": pf,
                "trades": hold.get("trades"),
                "live_cr": float(live_cr),
                "live_net_r": live_r,
                "live_pf": live_pf,
            }
    return best
...
def _score_cr(row: dict[str, Any], caps: tuple[float, ...]) -> dict[float, dict[str, Any] | None]:
    ...
    for cap in caps:
        ...
        for k in ("available", "digits", "description"):
            overlay.pop(k, None)
        overlay["cost_rank_max"] = float(cap)
...
def propose_cost_rank_upgrade(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        live_cr = float(row.get("cost_rank_max") or 0.0)
    except (TypeError, ValueError):
        return None
    caps = tuple(sorted(set(COST_RANK_CANDIDATES) | {live_cr}))
    return gate_pick(
        row, best_cost_rank_upgrade(live_cr, _score_cr(row, caps)),
        field="cost_rank_max", value_key="cost_rank_max")
...
def apply_cost_rank_upgrade(
    ...
    *,
    ...
    sym = str(row.get("symbol") or "")
    pick = propose_cost_rank_upgrade(row)
    if pick is None:
        try:
            cur = float(row.get("cost_rank_max") or 0.0)
        except (TypeError, ValueError):
            cur = 0.0
        return True, f"{sym} cost_rank degismedi ({cur:g})"

    cap = float(pick["cost_rank_max"])
    try:
    ...
    try:
        ...
        return False, f"{sym} cost_rank fail: {exc.read().decode()[:100]}"
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        return False, f"{sym} cost_rank fail: {exc}"

    return True, (
        f"{sym} cost_rank {pick['live_cr']:g}->{cap:g} "
