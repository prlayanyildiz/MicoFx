    ...
    *,
    ...
    for val, hold in scored.items():
        ...
        if best is None or key < best_key:
            best_key = key
            best = {
                "min_body_ratio": float(val),
                "net_r": net_r,
                "profit_factor": pf,
                "trades": hold.get("trades"),
                "live_body": float(live_val),
                "live_net_r": live_r,
                "live_pf": live_pf,
            }
    return best
...
def _score(row: dict[str, Any], vals: tuple[float, ...]) -> dict[float, dict[str, Any] | None]:
    ...
    for val in vals:
        ...
        for k in ("available", "digits", "description"):
            overlay.pop(k, None)
        overlay["min_body_ratio"] = float(val)
...
def propose_body_upgrade(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        live_val = float(row.get("min_body_ratio") or 0.0)
    except (TypeError, ValueError):
        return None
    vals = tuple(sorted(set(BODY_CANDIDATES) | {live_val}))
    return gate_pick(
        row, best_body_upgrade(live_val, _score(row, vals)),
        field="min_body_ratio", value_key="min_body_ratio")
...
def apply_body_upgrade(
    ...
    *,
    ...
    sym = str(row.get("symbol") or "")
    pick = propose_body_upgrade(row)
    if pick is None:
        try:
            cur = float(row.get("min_body_ratio") or 0.0)
        except (TypeError, ValueError):
            cur = 0.0
        return True, f"{sym} min_body_ratio degismedi ({cur:g})"

    val = float(pick["min_body_ratio"])
    try:
    ...
    try:
        ...
        return False, f"{sym} min_body_ratio fail: {exc.read().decode()[:100]}"
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        return False, f"{sym} min_body_ratio fail: {exc}"

    return True, (
        f"{sym} min_body_ratio {pick['live_body']:g}->{val:g} "
