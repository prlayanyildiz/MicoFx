    ...
    *,
    ...
    for val, hold in scored.items():
        ...
        if best is None or key < best_key:
            best_key = key
            best = {
                "atr_pct_min": float(val),
                "net_r": net_r,
                "profit_factor": pf,
                "trades": hold.get("trades"),
                "live_atr_pct": float(live_val),
                "live_net_r": live_r,
                "live_pf": live_pf,
            }
    return best
...
def propose_atr_pct_upgrade(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        live_val = float(row.get("atr_pct_min") or 0.0)
    except (TypeError, ValueError):
        return None
    vals = tuple(sorted(set(ATR_PCT_CANDIDATES) | {live_val}))
    return gate_pick(
        row, best_atr_pct_upgrade(live_val, _score(row, vals)),
        field="atr_pct_min", value_key="atr_pct_min")
...
def apply_atr_pct_upgrade(
    ...
    *,
    ...
    sym = str(row.get("symbol") or "")
    pick = propose_atr_pct_upgrade(row)
    if pick is None:
        try:
            cur = float(row.get("atr_pct_min") or 0.0)
        except (TypeError, ValueError):
            cur = 0.0
        return True, f"{sym} atr_pct_min degismedi ({cur:g})"

    val = float(pick["atr_pct_min"])
    try:
    ...
    try:
        ...
        return False, f"{sym} atr_pct_min fail: {exc.read().decode()[:100]}"
    except (urllib.error.URLError, TimeoutError, OSError) as exc:
        return False, f"{sym} atr_pct_min fail: {exc}"

    return True, (
        f"{sym} atr_pct_min {pick['live_atr_pct']:g}->{val:g} "
