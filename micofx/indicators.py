...
def wma(src: np.ndarray, length: int) -> np.ndarray:
    """Linear weighted MA; the most recent bar has the largest weight (1..n).

    Pine ``wma``. Expanding warmup on the head so a prefix of a longer series
    matches the same bars of a shorter one.
    """
    length = max(1, int(length))
    n = src.size
    out = np.empty(n, dtype=np.float64)
    if n == 0:
        return out
    values = src.astype(np.float64, copy=False)
    if length == 1:
        return values.copy()
    ...
    head = min(length - 1, n)
    for i in range(head):
        ww = np.arange(1, i + 2, dtype=np.float64)
        out[i] = float(np.dot(values[: i + 1], ww) / ww.sum())
    if n >= length:
        out[length - 1:] = sliding_window_view(values, length).dot(weights) / wsum
    return out


def alpha_trend_rsi(high: np.ndarray, low: np.ndarray, close: np.ndarray,
                    period: int = 14, coeff: float = 1.0) -> np.ndarray:
    """Kivanc AlphaTrend in RSI mode (Pine ``novolumedata=true``).

    The period is shared: SMA of true range *and* RSI. RSI >= 50 ratchets the
    line up against ``low - ATR*coeff``; otherwise it ratchets down against
    ``high + ATR*coeff``. This is not SuperTrend - the ATR here is SMA(TR),
    not Wilder, and the gate is RSI rather than a close-vs-band flip.
    """
    ...
    if n == 0:
        return np.empty(0, dtype=np.float64)
    atr_sma = sma(true_range(high, low, close), period)
    rsi_line = rsi(close, period)
    up_t = low.astype(np.float64, copy=False) - atr_sma * coeff
    down_t = high.astype(np.float64, copy=False) + atr_sma * coeff
    line = np.empty(n, dtype=np.float64)
    prev = 0.0
    for i in range(n):
        if rsi_line[i] >= 50.0:
            prev = prev if up_t[i] < prev else float(up_t[i])
        else:
            prev = prev if down_t[i] > prev else float(down_t[i])
        line[i] = prev
    return line


def mavilim_w(close: np.ndarray, first: int = 3, second: int = 5) -> np.ndarray:
    """Kivanc MavilimW: six nested WMAs with Fibonacci-sum lengths.

    Default 3 and 5 produce lengths 3-5-8-13-21-34. The slope of the outer
    line is the signal; this helper only returns the line.
    """
    fmal = max(1, int(first))
    smal = max(1, int(second))
    tmal = fmal + smal
    fmal2 = smal + tmal
    ftmal = tmal + fmal2
    smal2 = fmal2 + ftmal
    return wma(wma(wma(wma(wma(wma(close, fmal), smal), tmal), fmal2), ftmal), smal2)
